...
?length()
length(myname) # This object has a length of one. Why?
...
# Concept Check: How could we make myname longer than 1?
myname_2 <- c("Odum", "Ecology")
length(myname_2)
...
Matrices are a special type of collections of vectors. The data is organized into columns and rows giving the matrix dimensions. 
...
# Practice: Add more rows and columns to the matrix and fill it with 1 (one's) and change the names of the rows and columns
rnames <- c("R1", "R2", "R3")
cnames <- c("C1", "C2", "C3")
m <- matrix(data = 1, nrow = 3, ncol = 3, byrow = TRUE, dimnames = list(rnames, cnames))
m # now what does our matrix look like?
...
# A common way of subsetting values from a matrix is using bracket notation. 
# Note that, unlike vectors which are linear, here we use two values for subsetting - one for each dimension of the matrix
# Bracket notation follows the format: [row number, column number]
...
# Check the class of the 3rd entry of the "w" list
class(w[[3]])

# Determine the structure of the 8th element of the "primes" list
str(primes[[8]])
...
Factors are used for ordered (e.g. low, medium, high) or unordered (e.g. blue, red, green) categorical data, represented as either characteris or numerics. This data type is important for modeling and plotting work. Factors can only contain pre-defined values and are how we can add labels to integers. For example, if sampling sites are labeled as numbers (e.g. sites 1, 2, 3), we can use factors to tell `R` that these values are actually categorical rather than continuous.
...
class(dat)
print(is.factor(dat)) # nope! 

# Now convert the vector into a factor
dat <- factor(dat)
class(dat) # yep, now a factor!
...
However, as we saw in the previous section, the `R` community lead by Garrett Grolemund and Hadley Wickham has developed `tidyverse` work flows that are more efficient for common data science manipulations. Check out the Tidyverse and "R for Data Science" and "statmethods" for more information.
...
df[,1] # subsetting a column
df[1,] # subsetting a row
...
I got you started, but remember to add comments that describe what each line of code does!
